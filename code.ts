...
class colisionObject {
    ...
    vx:number;
    vy:number;
    vNormalized: [number, number]
    constructor(x:number, y:number, width:number, height:number, vx:number, vy:number){
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    isBorderColided(width:number, height:number){
    ...
    colisionCorrection(obj: colisionObject){
        do{
            this.x-=this.vNormalized[0];
            this.y-=this.vNormalized[1];
        }while(this.isColided(obj));
    }
    borderCorrection(width:number, height:number){
        do{
            this.x-=this.vNormalized[0];
            this.y-=this.vNormalized[1];
        }
        while(this.isBorderColided(width, height));
    }
    move(){
        this.x+=this.vx;
        this.y+=this.vy;
...
class DvDObject {
    ...
    setPositionInDom(){
        this.div.style.left = `${this.obj.vx}px`;
        this.div.style.top = `${this.obj.vy}px`;
    }
    changeColor(){
        this.svg.setAttribute('fill', this.randomColor());
    }
...
const dvds:DvDObject[] = []

for(let i = 0; i < 1; i++){
    dvds.push(new DvDObject);
}

const updateDVDs = async () => {
    dvds.map((dvd) => {
        dvd.obj.move();
        const colided = dvd.obj.isBorderColided(viewScreen.width, viewScreen.height);
        if(colided !== false){
            dvd.obj.x -= dvd.obj.vx;
            dvd.obj.y -= dvd.obj.vy;
